...
try:
    ...
    print("[ERROR]: The 'dateparser' library is not installed. Please install it with 'pip install dateparser' or use 'uv run --with dateparser'.", file=sys.stderr)
    sys.exit(1)
...
def main() -> None:
    ...
    parser.add_argument(
        "--time", 
        required=True, 
        help="Natural language time (e.g. 'in 6 hours', '9:30 AM', 'tomorrow at noon')"
    )
    parser.add_argument(
        "--message", 
        required=True, 
        help="The message to print to stdout when the time is up to wake the agent"
    )
    ...
    if not target_time:
        print(f"\n[ADVANCED-SCHEDULE-ERROR]: Could not parse time string '{args.time}'. Please try a different format.", flush=True, file=sys.stderr)
        sys.exit(1)

    now = datetime.now()
    if target_time < now:
        print(f"\n[ADVANCED-SCHEDULE-ERROR]: The parsed time '{target_time}' is in the past! Current time is {now}.", flush=True, file=sys.stderr)
        sys.exit(1)
